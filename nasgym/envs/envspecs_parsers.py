...
class ChainedEnvParser(AbstractEnvSpecsParser):
    """Default parser for the environment specs."""

    def __init__(self, config_file):
        """Constructor."""
        super(ChainedEnvParser, self).__init__(config_file=config_file)
        self.reload_configuration()
    ...
    def _populate_action_space(self, max_nlayers, nasenv_dict):
        ...
        for layer in nasenv_dict['layers']:
            for layer_key, layer_config in layer.items():
                ...
                pred1_bool = "L" if layer_config['pred1'] else 0

                pred2_bool = "BL" if layer_config['pred2'] else 0
                ...
                for kernel in kernels_list:
                    ...
                    action_type = \
                        "{type}_k-{kernel}_pred1-{pred1}_pred2-{pred2}".format(
                            type=layer_key,
                            kernel=kernel,
                            pred1=pred1_bool, pred2=pred2_bool
                        )
...
class PredecessorFreeEnvParser(AbstractEnvSpecsParser):
    ...
    def __init__(self, config_file):
        """Constructor."""
        super(PredecessorFreeEnvParser, self).__init__(config_file=config_file)
        self.reload_configuration()
    ...
    def _populate_action_space(self, max_nlayers, nasenv_dict):
        ...
        # Add the predshift operations (4 in total)
        for pred in range(1, 3):  # for i in [1, 2]
            for operation in ["U", "D"]:
                action_type = \
                    "predshift_p-{p}_op-{op}".format(p=pred, op=operation)
                action_info[counter] = action_type
                counter += 1
